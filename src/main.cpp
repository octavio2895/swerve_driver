...
typedef struct
{
  ...
  double rpm_motor;
  double target_rpm_motor = 0;
  double pos_motor;
  double target_pos_motor = 180;
...
int32_t dir, dead_zone_up = 450, dead_zone_down = 450, max_up = 1023, max_down = -1023; // Rango PWM
...
uint32_t curr_millis, can_update_millis, update_sensors_millis, blink_millis, update_encoder_millis, update_vel_pos_sp_millis, control_correction_millis, elapsed_millis, last_millis, print_serial_millis;
bool flag_can_rx;
...
void set_pid_speed(float speed)
{
  // Serial2.print("Target RPM was: ");
  // Serial2.println(sys_state.target_rpm_motor);
  sys_state.target_rpm_motor = speed;
  // Serial2.print("Target RPM now is: ");
  // Serial2.println(sys_state.target_rpm_motor);
}
void set_pid_pos(float pos)
{
  // Serial2.print("Target SETPOINT was: ");
  // Serial2.println(sys_state.target_pos_motor);
  sys_state.target_pos_motor = pos;
  // Serial2.print("Target SETPOINT is: ");
  // Serial2.println(sys_state.target_pos_motor);
}
void timeout_reset()
{
